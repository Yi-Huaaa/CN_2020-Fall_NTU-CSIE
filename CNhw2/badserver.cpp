...
int playing_video_ing [30][4];
char playing_video_flnm[30][BUFF_SIZE];

void * doInChildThread(void *arg){
    int i =  *(int*)arg;
    char filenm [BUFF_SIZE];
    for (int j = 0; j < BUFF_SIZE; j++)
        filenm[j] = playing_video_flnm[i][j];
    int remoteSocket = playing_video_ing[i][2];
    playing_video_ing[i][0] = 1;
...
int main(int argc , char *argv[]){
    ...
    for(int i = 0; i < max_clients; i++){
        client_socket[i] = 0;
        playing_video_ing[i][0] = 0; // 開始播影片
        playing_video_ing[i][1] = 0; //播完影片 
        playing_video_ing[i][2] = 0; // remoteSoket
        playing_video_ing[i][3] = 0; // i
    }
    pthread_t pid[30];
    ...
    while(1){ 
        ...
        for(int i = 0; i < max_clients; i++){
            sd = client_socket[i];
            //printf("1:::sd = %d\n", sd);
            if(FD_ISSET(sd, &readfds) && playing_video_ing[i][0] == 0){
                bzero(Buf, sizeof(char)* BUFF_SIZE); 
                ...
                if(Buf[0] == 'l' && Buf[1] == 's'){ //ls-1: OK!
                    ...
                    if(exist[0] == '1'){
                        ...
                        playing_video_ing[i][2] = sd;
                        playing_video_ing[i][3] = i;
                        for(int j = 0; j < BUFF_SIZE; j++)
                            playing_video_flnm[i][j] = filenm[j];
                        int* input = new int(i);
                ...
                for (int j = 0; j < max_clients; j++){
                    if(playing_video_ing[j][0] == 1 && playing_video_ing[j][1] == 1){
                        close( playing_video_ing[j][2] );
                        client_socket[ playing_video_ing[j][3] ] = 0;
                        pthread_join(pid[j], NULL);
                        playing_video_ing[j][0] = 0;
                        playing_video_ing[j][1] = 0;
                        playing_video_ing[j][2] = 0;
                        playing_video_ing[j][3] = 0;
                    }
                }
            }
        }
